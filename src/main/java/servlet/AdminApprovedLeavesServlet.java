...
@WebServlet("/admin/approved-leaves")
public class AdminApprovedLeavesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String selectQuery = "SELECT l.id, l.student_id, l.from_date, l.to_date, l.reason, l.status, l.remarks, l.applied_on, u.regNo "
					+ "FROM leave_applications l " + "LEFT JOIN userregistration u ON l.student_id = u.id "
					+ "WHERE l.status = 'Approved' " + "ORDER BY l.applied_on DESC";
		...
		request.getRequestDispatcher("/admin/approved-leaves.jsp").forward(request, response);
	}
}
